...
type KafkaConsumer struct {
	...
	TagForKey              string          `toml:"tag_for_key"`
...
func (h *ConsumerGroupHandler) Handle(session sarama.ConsumerGroupSession, msg *sarama.ConsumerMessage) error {
	...
	// for key-value pair message
	if h.TagForKey != "" && len(msg.Key) > 0 {
		for _, metric := range metrics {
			metric.AddTag(h.TagForKey, string(msg.Key))
			metric.AddTag("offset", strconv.FormatInt(msg.Offset, 10))
		}
	}
...
func init() {
	inputs.Add("ose_kafka_consumer", func() telegraf.Input {
